...
class URLExtract(CacheFile):
    ...
    **Examples:**
    ...
    def _reload_tlds_from_file(self):
        ...
        tlds = self._load_cached_tlds()
        tlds.update([ '.'+str(i) for i in range(256) ])
    ...
    def _complete_url(self, text, tld_pos, tld):
        ...
        self._match_indices.append({'url': complete_url,
            'start': start_pos, 'end': start_pos + len(complete_url)})
